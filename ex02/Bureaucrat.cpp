...
void Bureaucrat::signForm(AForm &form) _NOEXCEPT {
    try {
        form.beSigned(*this);
        std::cout << *this << " signed " << form << std::endl;
    } catch (AForm::GradeTooLowException &exception) {
        std::cerr << *this << " couldn't sign " << form << " because " << exception.what() << std::endl;
    }
}

void Bureaucrat::executeForm(const AForm &form) _NOEXCEPT {
    try {
        form.execute(*this);
        std::cout << *this << " executed " << form << std::endl;
    } catch (std::exception &exception) {
        std::cerr << *this << " couldn't execute " << form << " because " << exception.what() << std::endl;
    }
